...
import { IExercisePlanDay, IActivity } from "@/providers/fitnesspath/activity-provider/context";
...
const closeModal = () => {
  ...
  return (
    <div className="workout-plan-container" style={{ padding: "2rem", backgroundColor: "#f5f7fa" }}>
      <div className="header-container" style={{ marginBottom: 24 }}>
        <Title level={2} style={{ marginBottom: 8 }}>My 10-Day Workout Plan</Title>
        <Paragraph type="secondary">Follow this personalized plan to reach your fitness goals</Paragraph>
      </div>
            ...
            <Col xs={24} sm={12} md={8} lg={6} key={day.id}>
              <Badge.Ribbon 
                ...
                >
                  ...
                  >
                    {day.description || "Focus on form and controlled movements"}
                  </Paragraph>
      ...
      <Modal
        title={
          <div style={{ display: "flex", alignItems: "center" }}>
            <div style={{ 
              backgroundColor: selectedDay ? getDifficultyColor(selectedDay.difficulty) : 'blue', 
              width: 10, 
              height: 10, 
              borderRadius: 5, 
              marginRight: 8 
            }}></div>
            <span>Day {exercisePlan.findIndex(d => d.id === selectedDay?.id) + 1} Workout</span>
          </div>
        }
        open={isModalVisible}
        onCancel={closeModal}
      ...
      >
        {selectedDay ? (
          <div className="workout-detail">
            <div className="workout-header" style={{ 
              ...
              <div>
                ...
                <Title level={4} style={{ margin: "8px 0" }}>
                  {selectedDay.workoutType.charAt(0).toUpperCase() + selectedDay.workoutType.slice(1)} Workout
                </Title>
              </div>
            </div>
            ...
            <Paragraph>
              {selectedDay.description || "Focus on proper form and controlled movements throughout this workout."}
            </Paragraph>
              ...
              <ul className="activity-list" style={{ paddingLeft: 20 }}>
                {selectedDay.activities.map((activity: IActivity, i) => (
                  <li key={i} style={{ marginBottom: 12 }}>
                    <Text strong>{activity.description || `Exercise ${i+1}`}</Text>
                  </li>
                ))}
              </ul>
            ) : (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                <Text type="secondary">No specific activities defined for this day.</Text>
              </div>
            )}
          </div>
        ) : (
