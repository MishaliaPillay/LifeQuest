...
import { useStepsActions, useStepsState } from "@/providers/fitnesspath/step-provider";
...
export default function FitnessDashboard() {
  ...
  const fetchWorkoutPlan = async (id) => {
    try {
      ...
      const enhancedPlan = planResponse.map((day) => {
        ...
        const workoutType = day.description?.toLowerCase().includes('cardio') ? 'cardio' :
                          day.description?.toLowerCase().includes('strength') ? 'strength' :
                          day.description?.toLowerCase().includes('flex') ? 'flexibility' :
                          day.description?.toLowerCase().includes('recovery') ? 'recovery' : 'hiit';
  ...
  const workoutProgress = exercisePlan.length > 0
  ...
  return (
    <div style={{ padding: "24px", backgroundColor: "#f5f7fa", minHeight: "100vh" }}>
      <Title level={2} style={{ marginBottom: 24 }}>Fitness Dashboard</Title>
                  ...
                  <Col xs={24} md={12}>
                    <Card title="Steps Progress" bordered={false}>
                      <Progress
                        ...
                        status={stepsStats.weeklyProgress >= 100 ? "success" : "active"}
                        strokeColor={{
                          '0%': '#108ee9',
                          '100%': '#87d068',
                        }}
                      ...
                      <Text>
                        {stepsStats.weeklyProgress >= 100
                          ...
                          : `${10000 - stepsStats.weeklyAverage} more steps per day to reach your goal`}
                      </Text>
                  ...
                  <Col xs={24} md={12}>
                    <Card title="Workout Plan Progress" bordered={false}>
                      ...
                      <div style={{ marginTop: 24 }}>
                        ...
                        {loadingWorkouts ? (
                        ...
                        ) : exercisePlan.filter(day => !day.isComplete).length > 0 ? (
                          <div>
                            {exercisePlan
                                  ...
                                  <div style={{ display: "flex", alignItems: "center" }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                      ...
                                      <Text ellipsis style={{ width: '100%' }}>
                                        {day.description || `${day.workoutType.charAt(0).toUpperCase() + day.workoutType.slice(1)} workout`}
                                      </Text>
                                    </div>
                                    <div>
                          ...
                          <div style={{ textAlign: "center", padding: 24 }}>
                            <Text type="secondary">All workouts completed!</Text>
                          </div>
                        )}
                      </div>
                    </Card>
                  ...
                  <Col xs={24}>
                    <Card title="Recent Workouts" bordered={false}>
                      {loadingWorkouts ? (
                        ...
                        <div style={{ textAlign: "center", padding: 40 }}>
                          <Text type="secondary">No workout plan data available</Text>
                        </div>
                      )}
                    </Card>
                  </Col>
