...
import { Card, Row, Col, Progress, Statistic, Avatar, Badge, Tag, Typography, Space } from 'antd';
...
const XpLevelCard: React.FC<Props> = ({ name, level, xp, xpToNextLevel, badges }) => (
  <Card
    variant="borderless"
    style={{
  ...
  >
    <Row gutter={24} align="middle">
      <Col xs={24} sm={4} md={3} style={{ textAlign: 'center' }}>
        <Badge count={level} overflowCount={999}>
          <Avatar size={80} icon={<UserOutlined />} style={{ backgroundColor: '#fff', color: '#1890ff' }} />
        </Badge>
      </Col>

      <Col xs={24} sm={16} md={18}>
        <Title level={4} style={{ color: 'white', marginBottom: 5 }}>{name}</Title>
        <Text style={{ color: 'white', marginBottom: 10, display: 'block' }}>
          Level {level} Fitness Warrior
        </Text>
        <Progress
          percent={Math.round((xp / xpToNextLevel) * 100)}
          status="active"
          showInfo={false}
          strokeColor="#cb44b2"
        ...
        <Text style={{ color: 'white' }}>
          {xp.toLocaleString()} / {xpToNextLevel.toLocaleString()} XP to Level {level + 1}
        </Text>
      </Col>
